...
//   const rule = split[0].split("-");
//   const ruleMin = rule[0];
//   const ruleMax = rule[1];
//   const letter = split[1];
//   const pwd = split[2];
//   const pwdLetterLength = pwd.split(letter).length - 1;

//   if (pwdLetterLength >= ruleMin && pwdLetterLength <= ruleMax) {
//     validPasswords++;
...
for (const password of input) {
  ...
  const pos1 = Number(rule[0]) - 1;
  const pos2 = Number(rule[1]) - 1;

  const checkLetter = split[1];
  const passwordString = split[2];

  const passwordLettersArray = passwordString.split("");

  if (
    passwordLettersArray[pos1] == checkLetter ||
    passwordLettersArray[pos2] == checkLetter
  ) {
    if (passwordLettersArray[pos1] == passwordLettersArray[pos2]) {
      console.log("bad");
    } else {
      validPasswords++;
      console.log(validPasswords);
